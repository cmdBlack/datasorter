...
csv_file_prev = pathlib.Path("outputs/" + "monthly-" + frame.loc[0]["date"][:7] + "-" + station + ".csv")
...
for row in tqdm(range(len(frame.recno))):
    ...
    if csv_file_prev != csv_file:

        summarize(frame, csv_file_prev)


    else:
        pass

    csv_file_prev = csv_file
...
for row in tqdm(range(len(frame.recno))):
    ...
    #wl_data = frame.loc[row]["wl_data"]
    #date = frame.loc[row]["wl_data"]
    #time = frame.loc[row]["wl_data"]
    ...
    if csv_file_prev != csv_file:
        data_prev = pd.read_csv(csv_file_prev) 
        frame2 = pd.DataFrame(data_prev)
        del frame2["nodeid"]
        del frame2["logtype"]
        del frame2["unit"]
