...
now = datetime.strptime("2025-05-02 08:03:00", "%Y-%m-%d %H:%M:%S")
...
for row in tqdm(frame2.index):
    ...
    #if (now - pd.to_timedelta(9, unit='h')).strftime("%Y-%m") == frame2.loc[row]['logtime'][:7]:
    if now.strftime("%d")  == '01':
        # get last month data
        if (now - pd.to_timedelta(9, unit='h')).strftime("%Y-%m") == frame2.loc[row]['logtime'][:7] or now.strftime("%Y-%m") == frame2.loc[row]['logtime'][:7]:
        ...
        if now.strftime("%Y-%m") == frame2.loc[row]['logtime'][:7]:
            if str(frame2.loc[row]["nodeid"]) in station_number.values():
